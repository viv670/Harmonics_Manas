...
class StandardPattern:
    ...
    def _generate_hash(self) -> str:
        """Generate unique pattern hash based on points"""
        points = []
        if self.x_point:
            points.append(f"X{self.x_point.index}_{self.x_point.price:.2f}")
        if self.a_point:
            points.append(f"A{self.a_point.index}_{self.a_point.price:.2f}")
        if self.b_point:
            points.append(f"B{self.b_point.index}_{self.b_point.price:.2f}")
        if self.c_point:
            points.append(f"C{self.c_point.index}_{self.c_point.price:.2f}")
        if self.d_point:
            points.append(f"D{self.d_point.index}_{self.d_point.price:.2f}")
