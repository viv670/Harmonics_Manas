...
class PatternDetectionWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if 'formed_abcd' in self.pattern_types:
                ...
                import sys
                print(f"DEBUG: Starting formed_abcd at {self.time.time()}", flush=True)
                sys.stdout.flush()
                results['formed_abcd'] = self.detect_formed_abcd_patterns()
                print(f"DEBUG: Finished formed_abcd at {self.time.time()}, found {len(results['formed_abcd'])} patterns")
                if DEBUG_MODE: print(f"Found {len(results['formed_abcd'])} Formed ABCD patterns")
                print("DEBUG: About to emit 30% progress")
                self.progress.emit(30)
                print("DEBUG: Emitted 30% progress")
            ...
            if 'comprehensive_abcd' in self.pattern_types:
                ...
                print(f"DEBUG: About to call detect_comprehensive_abcd_patterns at {self.time.time()}", flush=True)
                print(f"DEBUG: Working with {len(self.extremum_points)} extremum points", flush=True)
                import sys
                sys.stdout.flush()
                results['comprehensive_abcd'] = self.detect_comprehensive_abcd_patterns()
                print(f"DEBUG: Finished detect_comprehensive_abcd_patterns at {self.time.time()}", flush=True)
    ...
    def detect_formed_xabcd_patterns(self):
        ...
        print("\n" + "="*60)
        print("DEBUG: detect_formed_xabcd_patterns STARTED")
        print("="*60)
    ...
    def detect_comprehensive_xabcd_patterns(self):
        ...
        print("\n" + "="*60)
        print("DEBUG: detect_comprehensive_xabcd_patterns STARTED")
        print("="*60)
        ...
        print(f"DEBUG: Using search window: {max_window}")
    ...
    def detect_unformed_xabcd_patterns(self):
        ...
        print("\n" + "="*60)
        print("DEBUG: detect_unformed_xabcd_patterns STARTED")
        print("="*60)
...
class PatternViewerWindow(QMainWindow):
    ...
    def updateDetails(self, pattern):
        ...
        if 'prz_zones' in pattern and 'D' in pattern.get('points', {}) and 'X' not in pattern.get('points', {}):
            details += "PRZ Zones (Formed ABCD Pattern):\n"
            for i, zone in enumerate(pattern['prz_zones'], 1):
                ...
                if 'AB=CD_bull_' in pattern_source:
                    ...
                    simplified_name = pattern_source
                details += f"  PRZ ({simplified_name}): {zone['min']:.2f} - {zone['max']:.2f}\n"
                details += f"    Projection: {zone['proj_min']:.1f}% - {zone['proj_max']:.1f}%\n"
        ...
        if 'd_lines' in pattern and 'D' in pattern.get('points', {}) and 'X' in pattern.get('points', {}):
            d_lines = pattern['d_lines']
            details += f"D Lines (Formed XABCD Pattern - {len(d_lines)} lines):\n"
            for i, d_price in enumerate(d_lines, 1):
                details += f"  D{i}: {d_price:.2f}\n"
...
class HarmonicPatternDetector(QMainWindow):
    ...
    def initUI(self):
        ...
        search_window_layout.addWidget(QLabel("Search Window:"))

        # Add permanent label on the left showing 0=Auto
        left_label = QLabel("0=Auto")
        left_label.setStyleSheet("color: #606060; font-size: 9px;")
        search_window_layout.addWidget(left_label)

        self.search_window_spinbox = QSpinBox()
        self.search_window_spinbox.setMinimum(0)  # 0 means auto/adaptive
        self.search_window_spinbox.setMaximum(9999)  # Allow very large values, 9999 = unlimited
        self.search_window_spinbox.setValue(0)  # Default to auto
        self.search_window_spinbox.setToolTip("Maximum distance between pattern points:\n0 = Auto (adaptive based on dataset size)\n1-9998 = Custom limit (bars)\n9999 = Unlimited (search entire dataset)")
        self.search_window_spinbox.setSpecialValueText("Auto")  # Display "Auto" when value is 0

        # Custom display for value 9999
        original_textFromValue = self.search_window_spinbox.textFromValue
        def customTextFromValue(value):
            if value == 9999:
                return "Unlimited"
            return original_textFromValue(value)
        self.search_window_spinbox.textFromValue = customTextFromValue
        search_window_layout.addWidget(self.search_window_spinbox)

        # Add permanent label on the right showing 9999=Unlimited
        right_label = QLabel("9999=Unlimited")
        right_label.setStyleSheet("color: #606060; font-size: 9px;")
        search_window_layout.addWidget(right_label)

        # Add dynamic info label that shows current status
        self.search_window_info_label = QLabel("")
        self.search_window_info_label.setStyleSheet("color: gray; font-size: 10px;")
        search_window_layout.addWidget(self.search_window_info_label)

        # Connect to update the dynamic label when value changes
        self.search_window_spinbox.valueChanged.connect(self.updateSearchWindowLabel)

        search_window_layout.addStretch()  # Add stretch to align left
        pattern_layout.addLayout(search_window_layout)
    ...
    def downloadBinanceData(self):
        ...
        try:
            ...
            import traceback
            error_details = f"Failed to download data: {str(e)}\n\nDetails:\n{traceback.format_exc()}"
            print(error_details)  # Print to console for debugging
            QMessageBox.critical(self, "Download Error", f"Failed to download data for {symbol}:\n\n{str(e)}")
    ...
    def detectExtremums(self):
        ...
        test_bars = [233, 249, 252, 258]
        detected_test_bars = [b for b in test_bars if b in bar_indices]
        missing_test_bars = [b for b in test_bars if b not in bar_indices]
        print(f"DEBUG EXTREMUM: Bars 233,249,252,258")
        print(f"  Detected: {detected_test_bars}")
        print(f"  Missing: {missing_test_bars}")
    ...
    def updateSearchWindowLabel(self, value):
        """Update the search window info label based on current value"""
        if value == 0:
            self.search_window_info_label.setText("← Active")
            self.search_window_info_label.setStyleSheet("color: green; font-size: 10px; font-weight: bold;")
        elif value == 9999:
            self.search_window_info_label.setText("← Active")
            self.search_window_info_label.setStyleSheet("color: orange; font-size: 10px; font-weight: bold;")
        else:
            self.search_window_info_label.setText(f"(Custom: {value} bars)")
            self.search_window_info_label.setStyleSheet("color: blue; font-size: 10px;")
