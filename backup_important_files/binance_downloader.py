...
class BinanceDataDownloader:
    ...
    def download_data(self,
        ...
        try:
            ...
            response = self.session.get(symbol_info_url, params={'symbol': symbol}, timeout=10)
            if response.status_code != 200:
                raise ValueError(f"Symbol {symbol} not found on Binance. Please check the symbol name.")
        except requests.exceptions.RequestException as e:
            print(f"Warning: Could not verify symbol (network issue): {e}")
                ...
                if response.status_code == 200:
                    ...
                    if not klines:
                        ...
                        if request_count == 1 and current_start < end_ms:
                            ...
                            actual_start = self._find_first_available_date(symbol, interval, current_start, end_ms)
                            if actual_start:
    ...
    def _find_first_available_date(self, symbol: str, interval: str, start_ms: int, end_ms: int) -> Optional[int]:
        """
