...
class PatternTracker:
            ...
            if isinstance(d_proj, dict):
                ...
                if 'd_lines' in d_proj:
                    ...
                    if d_lines and not prz_min:
                        # FIX: Properly validate numeric d_lines (including negative values)
                        valid_d_lines = []
                        for d in d_lines:
                            if d is not None:
                                try:
                                    valid_d_lines.append(float(d))
    ...
    def check_price_in_zone(self, price_high: float, price_low: float, current_bar: int,
        ...
        for pattern_id, tracked in self.tracked_patterns.items():
            ...
            if zone_entered:
                # The projected zones were mathematically calculated to contain all valid D points
                # for this specific pattern type. No additional validation needed.
                # The zone IS the validation - if price is in zone, ratios are valid by definition.

                # Create a zone entry record for accurate success rate tracking
                is_bullish = tracked.a_point[1] > tracked.b_point[1]

                # Determine zone boundaries for this entry
                if tracked.pattern_type == 'ABCD':
                    ...
                    d_values = [float(d) for d in tracked.d_lines]
                    zone_min = min(d_values)
                    zone_max = max(d_values)

                # Create unique entry ID
                entry_id = f"{pattern_id}_{current_bar}_{entry_price:.2f}"
        ...
        for pattern_id, tracked in self.tracked_patterns.items():
            ...
            if tracked.status == 'in_zone':
                ...
                if 'reversal_confirmed' in locals() and reversal_confirmed:
                    ...
                    for entry_id in tracked.zone_entries:
                        if entry_id in self.zone_entries and self.zone_entries[entry_id].status == 'active':
                            self.zone_entries[entry_id].status = 'failed'
                            self.zone_entries[entry_id].exit_bar = current_bar
    ...
    def get_completion_statistics(self) -> Dict:
        ...
        total_failed_entries = sum(1 for e in self.zone_entries.values() if e.status == 'failed')
        total_active_entries = sum(1 for e in self.zone_entries.values() if e.status == 'active')
        total_concluded_entries = total_success_entries + total_failed_entries
        ...
        return {
            ...
            'failed_zone_entries': total_failed_entries,  # NEW
