...
class BacktestStatistics:
    ...
    patterns_invalid_prz: int = 0  # Patterns with invalid PRZ (single candle cross)
...
class OptimizedWalkForwardBacktester:
    ...
    def initialize_fibonacci_tracking(self, pattern: Dict, detection_bar: int):
        ...
        # Fibonacci tracking added (debug output removed)
    ...
    def run_backtest(self, progress_callback=None) -> BacktestStatistics:
        ...
        # Backtest starting (verbose output removed for cleaner console)
        ...
        if self.validate_d_crossing_during_tracking != DETECTION_VALIDATE_D_CROSSING:
            print("⚠️  WARNING: D-crossing validation mismatch between detection and tracking")
        ...
        for idx in range(start_idx, len(self.data)):
            ...
            # Progress bar removed for cleaner console
        ...
        print(f"  Invalid PRZ: {tracking_stats['invalid_prz']}")
        print(f"  Failed PRZ: {tracking_stats['failed_prz']}")
    ...
    def calculate_statistics(self) -> BacktestStatistics:
        ...
        stats.patterns_failed = tracking_stats['failed_prz']
        stats.patterns_invalid_prz = tracking_stats['invalid_prz']
        stats.patterns_failed_prz = tracking_stats['failed_prz']
