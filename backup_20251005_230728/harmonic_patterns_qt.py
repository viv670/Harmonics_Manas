...
from PyQt6.QtWidgets import (
    ...
    QDialog, QDialogButtonBox, QDoubleSpinBox, QScrollArea, QFrame, QRadioButton
)
...
# Watchlist and auto-update modules
...
# Pattern detection function placeholders
# These will be set to the actual functions below
detect_unformed_abcd_patterns = None
detect_formed_abcd_patterns = None
detect_formed_xabcd_patterns = None
detect_unformed_xabcd_patterns = None

# Import formed ABCD pattern detection (formerly called strict)
# These imports have been consolidated into formed_abcd.py and formed_xabcd.py

# Import ABCD pattern detection from separated modules
from formed_abcd import (
    detect_strict_abcd_patterns as detect_comprehensive_strict_abcd
)
from unformed_abcd import (
    detect_unformed_abcd_patterns as detect_comprehensive_unformed_abcd
)
print("ABCD pattern detection loaded")
...
detect_formed_abcd_patterns = detect_comprehensive_strict_abcd
detect_unformed_xabcd_patterns = detect_comprehensive_unformed_xabcd
detect_formed_xabcd_patterns = detect_formed_xabcd_func
...
class DataDownloadWorker(QThread):
    ...
    finished = pyqtSignal(object, str, str)  # dataframe, symbol, timeframe
    ...
    def run(self):
        ...
        try:
            ...
            base_timeframe = downloader.get_base_timeframe(self.timeframe)
            needs_resampling = (base_timeframe != self.timeframe)

            # Progress callback
            def update_progress(percent, message):
                self.progress.emit(percent, message)

            # Download at base timeframe
            ...
            # Resample if needed
            if needs_resampling:
                self.progress.emit(95, f"Resampling from {base_timeframe} to {self.timeframe}...")
                df = downloader.resample_ohlc(df, self.timeframe)

            # Emit success
            self.finished.emit(df, self.symbol, self.timeframe)
...
class PatternDetectionWorker(QThread):
    ...
    def __init__(self, extremum_points, pattern_types, data=None, main_window=None, validate_d_crossing=True):
        super().__init__()
        self.extremum_points = extremum_points
        self.pattern_types = pattern_types
        self.data = data  # Candlestick data for filtering unformed patterns
        self.main_window = main_window
        self.validate_d_crossing = validate_d_crossing  # D-point crossing validation toggle
        import time
    ...
    def run(self):
        ...
        try:
            ...
            if 'formed_abcd' in self.pattern_types:
                ...
                import sys
                results['formed_abcd'] = self.detect_formed_abcd_patterns()
                if DEBUG_MODE: print(f"Found {len(results['formed_abcd'])} Formed ABCD patterns")
            ...
            if 'formed_xabcd' in self.pattern_types:
                ...
                results['formed_xabcd'] = self.detect_formed_xabcd_patterns()
    ...
    def get_search_window(self, dataset_size):
        ...
        # Access the main window's spinbox
        ...
        # Auto mode (gui_window == 0) - use adaptive settings based on dataset size
        if dataset_size < 100:
            return None  # No limit for small datasets
    ...
    def detect_formed_xabcd_patterns(self):
        ...
        print(f"  - Max patterns: {max_pats}")
        print(f"  - Max window: {max_window}")

        # Call formed XABCD detection (caching makes it fast)
        ...
        print(f"DEBUG: detect_formed_xabcd_patterns returned {len(result) if isinstance(result, list) else 'NOT A LIST'} patterns")
        return result
    ...
    def filter_unformed_patterns(self, patterns, is_xabcd=False):
        ...
        print(f"DEBUG: filter_unformed_patterns starting with {len(patterns)} patterns")
        print(f"DEBUG: is_xabcd = {is_xabcd}")
        print(f"DEBUG: Data shape: {self.data.shape if self.data is not None else 'None'}")

        if len(patterns) > 0:
            print(f"DEBUG: First pattern keys: {list(patterns[0].keys())}")
            if 'points' in patterns[0]:
                print(f"DEBUG: Points keys: {list(patterns[0]['points'].keys())}")
                if 'D_projected' in patterns[0]['points']:
                    print(f"DEBUG: D_projected keys: {list(patterns[0]['points']['D_projected'].keys())}")

        filtered = []
        errors = []
        for i, pattern in enumerate(patterns):
            if i % 10 == 0:
                print(f"DEBUG: Filtering pattern {i}/{len(patterns)}")
            try:
                # Get C point time
                ...
                if i == 0:  # Debug first pattern in detail
                    print(f"DEBUG: First pattern C time: {c_time} (type: {type(c_time)})")
                    print(f"DEBUG: First pattern type: {is_bullish}")
                    print(f"DEBUG: D_projected content: {d_projected}")

                # Get candlesticks after C
                # For XABCD patterns, 'time' might actually be the bar index
                ...
                # Handle different formats: d_lines (XABCD), prz_zones, or single price
                if 'd_lines' in d_projected:
                    # XABCD format with d_lines
                    d_lines = d_projected['d_lines']
                    if d_lines and len(d_lines) > 0:
                        # Use the first D line as the primary target
                ...
                errors.append(str(e))
                print(f"DEBUG: Error processing pattern {i}: {e}")
                continue

        if errors:
            print(f"DEBUG: Total errors during filtering: {len(errors)}")
            if len(errors) > 0:
                print(f"DEBUG: First error: {errors[0]}")

        print(f"DEBUG: filter_unformed_patterns returning {len(filtered)} patterns")
        return filtered
    ...
    def detect_unformed_patterns(self):
        ...
        # When search window is unlimited (9999), use ALL extremum points
        if max_window is None:
            limited_extremum_points = self.extremum_points
        else:
            # Otherwise use adaptive limits
            if len(self.extremum_points) < 100:
                limited_extremum_points = self.extremum_points
            else:
    ...
    def detect_comprehensive_abcd_patterns(self):
        ...
        print(f"DEBUG: detect_comprehensive_abcd_patterns STARTED", flush=True)
        print(f"DEBUG: Total extremum points: {len(self.extremum_points)}", flush=True)
        print(f"DEBUG: Data shape: {self.data.shape if self.data is not None else 'None'}", flush=True)

        # Get search window using the helper method
        # IMPORTANT: Use extremum count, not dataset size, for unformed patterns
        # More extremum points need larger window to avoid missing patterns
        extremum_count = len(self.extremum_points)
        max_window = self.get_search_window(extremum_count)

        # Override: Ensure minimum window of 30 for unformed patterns to maintain accuracy
        if max_window is not None and max_window < 30:
            max_window = 30

        print(f"DEBUG: Using search window: {max_window} (extremum points: {extremum_count})", flush=True)

        # When search window is unlimited (9999), use ALL extremum points
        if max_window is None:
            limited_extremum_points = self.extremum_points
            max_pats = None  # No pattern limit when unlimited
            print(f"DEBUG: Unlimited search window, using ALL {len(self.extremum_points)} points", flush=True)
        else:
            # Adaptive limits based on dataset size
            if len(self.extremum_points) < 100:
                ...
                print(f"DEBUG: Small dataset ({len(self.extremum_points)} points), using NO limits", flush=True)
            elif len(self.extremum_points) < 500:
                # Medium dataset - use moderate limits
                limited_extremum_points = self.extremum_points[-300:] if len(self.extremum_points) > 300 else self.extremum_points
                max_pats = 200
                print(f"DEBUG: Medium dataset, using moderate limits", flush=True)
            else:
                # Large dataset - use balanced limits
                limited_extremum_points = self.extremum_points[-200:] if len(self.extremum_points) > 200 else self.extremum_points
                max_pats = 100
                print(f"DEBUG: Large dataset, using balanced limits", flush=True)

        print(f"DEBUG: Limited to {len(limited_extremum_points)} extremum points", flush=True)

        print(f"DEBUG: Calling detect_comprehensive_unformed_abcd with:", flush=True)
        print(f"  - points: {len(limited_extremum_points)}", flush=True)
        print(f"  - max_patterns: {max_pats}", flush=True)
        print(f"  - max_search_window: {max_window}", flush=True)
        import sys
        sys.stdout.flush()

        # Detect strict unformed ABCD patterns with adaptive limits
        unformed_patterns = detect_comprehensive_unformed_abcd(
            limited_extremum_points,
            self.data,
            log_details=True,  # Enable logging to see what's happening
            max_search_window=max_window  # Adaptive window
        )

        print(f"DEBUG: detect_comprehensive_unformed_abcd returned {len(unformed_patterns)} patterns")

        # Filter unformed patterns
        print(f"DEBUG: About to filter {len(unformed_patterns)} patterns")
        unformed_filtered = self.filter_unformed_patterns(unformed_patterns, is_xabcd=False)
        print(f"DEBUG: After filtering, have {len(unformed_filtered)} patterns")
    ...
    def detect_comprehensive_xabcd_patterns(self):
        ...
        # Get search window using the helper method
        dataset_size = len(self.data) if self.data is not None else len(self.extremum_points)
        max_window = self.get_search_window(dataset_size)

        # When search window is unlimited (9999), use ALL extremum points
        if max_window is None:
            limited_extremum_points = self.extremum_points
            max_pats = None  # No pattern limit when unlimited
            print(f"DEBUG: Unlimited search window, using ALL {len(self.extremum_points)} points")
        else:
            # Adaptive limits based on dataset size
            if len(self.extremum_points) < 100:
                limited_extremum_points = self.extremum_points
        ...
        # Detect strict unformed XABCD patterns with adaptive limits
        print(f"DEBUG: Calling detect_comprehensive_unformed_xabcd with:")
        print(f"  - Extremum points: {len(limited_extremum_points)}")
        print(f"  - Max patterns: {max_pats}")
        print(f"  - Max window: {max_window}")
        ...
        print(f"DEBUG: Got {len(unformed_patterns) if isinstance(unformed_patterns, list) else 'NOT A LIST'} patterns before filtering")

        # Filter unformed patterns
        unformed_filtered = self.filter_unformed_patterns(unformed_patterns, is_xabcd=True)

        print(f"DEBUG: After filtering: {len(unformed_filtered) if isinstance(unformed_filtered, list) else 'NOT A LIST'} patterns")
        return unformed_filtered
    ...
    def detect_unformed_xabcd_patterns(self):
        ...
        for i, ep in enumerate(extremums_to_use[:5]):  # Show first 5
            time, price, is_high = ep[0], ep[1], ep[2]  # Ignore bar_index ep[3]
        ...
        # Use adaptive limits - call the comprehensive version
        print(f"DEBUG: About to call detect_comprehensive_unformed_xabcd")
        print(f"  - max_pats in locals: {'max_pats' in locals()}")
        print(f"  - max_window in locals: {'max_window' in locals()}")
        if 'max_pats' in locals():
            print(f"  - max_pats value: {max_pats}")
        if 'max_window' in locals():
            print(f"  - max_window value: {max_window}")
        ...
        print(f"DEBUG: Got {len(patterns)} patterns from detect_comprehensive_unformed_xabcd")
...
class AllPatternsWindow(QMainWindow):
    ...
    def __init__(self, parent, data, patterns, extremum_points):
        ...
        self.parent_window = parent  # Store parent reference for re-detection
        self.data = data
    ...
    def createFilterSidebar(self):
        ...
        formed_count = self.pattern_counts.get('formed_abcd', 0)
        self.formed_abcd_checkbox = QCheckBox(f"Formed ABCD ({formed_count})")
        self.formed_abcd_checkbox.setChecked(True)  # Start checked
        self.formed_abcd_checkbox.setStyleSheet("QCheckBox { color: #000000; background-color: transparent; font-weight: bold; }")
        self.formed_abcd_checkbox.stateChanged.connect(self.onPatternTypeChanged)
        type_layout.addWidget(self.formed_abcd_checkbox)

        # Formed XABCD checkbox
        formed_xabcd_count = self.pattern_counts.get('formed_xabcd', 0)
        self.formed_xabcd_checkbox = QCheckBox(f"Formed XABCD ({formed_xabcd_count})")
        self.formed_xabcd_checkbox.setChecked(True)  # Start checked
        self.formed_xabcd_checkbox.setStyleSheet("QCheckBox { color: #000000; background-color: transparent; font-weight: bold; }")
        self.formed_xabcd_checkbox.stateChanged.connect(self.onPatternTypeChanged)
        type_layout.addWidget(self.formed_xabcd_checkbox)
        ...
        # D-Point Crossing Validation checkbox (for formed patterns only)
        self.validate_d_crossing_checkbox = QCheckBox("Strict D-Point Validation (Formed Only)")
        self.validate_d_crossing_checkbox.setChecked(True)  # Default to strict mode
        self.validate_d_crossing_checkbox.setToolTip(
            "Applies to FORMED patterns only:\n\n"
            "When ENABLED (checked):\n"
            "  • Hides formed patterns if price crossed D after formation\n"
            "  • Shows only patterns where reversal held perfectly\n"
            "  • Best for: Conservative analysis, backtesting\n\n"
            "When DISABLED (unchecked):\n"
            "  • Shows all formed patterns, even if D was violated\n"
            "  • Best for: Live trading, comprehensive pattern study\n\n"
            "Note: Unformed patterns are not affected by this setting"
        )
        self.validate_d_crossing_checkbox.setStyleSheet("QCheckBox { color: #0066CC; background-color: transparent; font-weight: bold; font-size: 11px; }")
        self.validate_d_crossing_checkbox.stateChanged.connect(self.onDPointValidationChanged)
        type_layout.addWidget(self.validate_d_crossing_checkbox)
    ...
    def onDPointValidationChanged(self):
        """Handle D-Point validation checkbox change - re-detect formed patterns"""
        from PyQt6.QtWidgets import QMessageBox

        if not hasattr(self, 'validate_d_crossing_checkbox'):
            return

        is_strict = self.validate_d_crossing_checkbox.isChecked()
        mode = "STRICT" if is_strict else "PERMISSIVE"

        # Ask user if they want to re-run detection
        msg = QMessageBox(self)
        msg.setWindowTitle("D-Point Validation Mode Changed")
        msg.setIcon(QMessageBox.Icon.Question)

        if is_strict:
            msg.setText(f"Mode changed to: {mode}\n\n"
                       "This will show only formed patterns where point D was not violated.\n\n"
                       "Do you want to re-run pattern detection now?")
        else:
            msg.setText(f"Mode changed to: {mode}\n\n"
                       "This will show ALL formed patterns, even if D was crossed.\n\n"
                       "Do you want to re-run pattern detection now?")
        ...
        if result == QMessageBox.StandardButton.Yes:
            # Close this window and trigger re-detection in parent
            if hasattr(self, 'parent_window') and self.parent_window:
                # Store the setting in parent window
                if not hasattr(self.parent_window, 'validate_d_crossing_setting'):
                    self.parent_window.validate_d_crossing_setting = is_strict
                else:
                    self.parent_window.validate_d_crossing_setting = is_strict
                ...
                # Trigger re-detection in parent
                self.parent_window.detectPatterns()
    ...
    def applyFilters(self):
        ...
        for pattern in self.all_patterns:
            ...
            if ptype == 'formed_abcd' and not hasattr(self, 'formed_abcd_checkbox'):
                continue
            if ptype == 'formed_abcd' and hasattr(self, 'formed_abcd_checkbox') and not self.formed_abcd_checkbox.isChecked():
                continue
            if ptype == 'formed_xabcd' and not hasattr(self, 'formed_xabcd_checkbox'):
                continue
            if ptype == 'formed_xabcd' and hasattr(self, 'formed_xabcd_checkbox') and not self.formed_xabcd_checkbox.isChecked():
                continue
...
class PatternViewerWindow(QMainWindow):
    ...
    def __init__(self, parent, patterns, data, extremum_points):
        ...
        # Get validate_d_crossing setting from parent (default True if not set)
        self.parent_validate_d_crossing = getattr(parent, 'validate_d_crossing_setting', True) if parent else True

        # Check if we have any formed patterns (for D-point validation checkbox visibility)
        self.has_formed_patterns = any(ptype in ['formed_abcd', 'formed_xabcd'] for ptype in patterns.keys() if patterns[ptype])
    ...
    def initUI(self):
        ...
        # Fibonacci Retracement Levels Toggle with customizable levels
        fib_group = QGroupBox("Fibonacci Retracement Levels")
        fib_group.setCheckable(True)
        fib_group.setChecked(False)  # Off by default
        fib_group.setToolTip(
            "Display Fibonacci retracement levels on FORMED patterns only\n"
            "Bullish: From highest formation point (A or C) down to D\n"
            "Bearish: From lowest formation point (A or C) up to D\n"
            "Golden Ratio zone (50%-61.8%) highlighted in gold"
        )
        fib_group.toggled.connect(self.refreshCurrentPattern)
        fib_group_layout = QVBoxLayout(fib_group)
        fib_group_layout.setContentsMargins(10, 10, 10, 10)
        fib_group_layout.setSpacing(5)

        # Store main checkbox reference for compatibility
        self.show_fibonacci_checkbox = fib_group

        # Default Fibonacci levels
        self.default_fib_levels = ['0', '23.6', '38.2', '50', '61.8', '78.6', '100']

        # Custom levels input
        input_row = QHBoxLayout()
        input_label = QLabel("Levels (%):")
        input_label.setStyleSheet("font-size: 10px; font-weight: bold;")
        input_row.addWidget(input_label)

        self.fib_levels_input = QLineEdit()
        self.fib_levels_input.setText(', '.join(self.default_fib_levels))
        self.fib_levels_input.setPlaceholderText("e.g., 0, 23.6, 38.2, 50, 61.8, 78.6, 100")
        self.fib_levels_input.setToolTip(
            "Enter Fibonacci levels as percentages separated by commas.\n"
            "Example: 0, 23.6, 38.2, 50, 61.8, 78.6, 100\n"
            "You can add any custom levels you want."
        )
        self.fib_levels_input.setStyleSheet("QLineEdit { font-size: 10px; }")
        input_row.addWidget(self.fib_levels_input)

        apply_btn = QPushButton("Apply")
        apply_btn.setMaximumWidth(60)
        apply_btn.setStyleSheet("QPushButton { font-size: 10px; padding: 3px; }")
        apply_btn.clicked.connect(self.applyFibonacciLevels)
        input_row.addWidget(apply_btn)

        fib_group_layout.addLayout(input_row)

        # Quick presets
        presets_row = QHBoxLayout()
        presets_label = QLabel("Presets:")
        presets_label.setStyleSheet("font-size: 9px; color: #666;")
        presets_row.addWidget(presets_label)

        preset_standard = QPushButton("Standard")
        preset_standard.setMaximumWidth(70)
        preset_standard.setStyleSheet("QPushButton { font-size: 9px; padding: 2px; }")
        preset_standard.clicked.connect(lambda: self.loadFibPreset('standard'))
        presets_row.addWidget(preset_standard)

        preset_extended = QPushButton("Extended")
        preset_extended.setMaximumWidth(70)
        preset_extended.setStyleSheet("QPushButton { font-size: 9px; padding: 2px; }")
        preset_extended.clicked.connect(lambda: self.loadFibPreset('extended'))
        presets_row.addWidget(preset_extended)

        preset_minimal = QPushButton("Minimal")
        preset_minimal.setMaximumWidth(70)
        preset_minimal.setStyleSheet("QPushButton { font-size: 9px; padding: 2px; }")
        preset_minimal.clicked.connect(lambda: self.loadFibPreset('minimal'))
        presets_row.addWidget(preset_minimal)

        presets_row.addStretch()
        fib_group_layout.addLayout(presets_row)

        # Add note about golden ratio
        golden_note = QLabel("Note: 50%-61.8% = Golden Ratio Zone")
        golden_note.setStyleSheet("font-size: 9px; color: #666; font-style: italic;")
        fib_group_layout.addWidget(golden_note)

        chart_layout.addWidget(fib_group)

        # Pattern details text
        self.details_text = QTextEdit()
        self.details_text.setReadOnly(True)
        self.details_text.setMaximumHeight(120)
        self.details_text.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap)  # Disable line wrapping to show horizontally
        ...
        self.bull_checkbox.stateChanged.connect(self.applyFilters)
        self.bull_label = QLabel("(0/0)")
        self.bull_label.setStyleSheet("color: #006400; font-weight: bold; font-size: 14px; margin-left: 8px; background-color: transparent;")  # Darker green for better visibility
        self.bull_label.setMinimumWidth(60)  # Wider to fit (X/Y) format
        ...
        self.bear_checkbox.stateChanged.connect(self.applyFilters)
        self.bear_label = QLabel("(0/0)")
        self.bear_label.setStyleSheet("color: #B22222; font-weight: bold; font-size: 14px; margin-left: 8px; background-color: transparent;")  # Darker red for better visibility
        self.bear_label.setMinimumWidth(60)  # Wider to fit (X/Y) format
        ...
        # D-Point Validation section (for formed patterns only)
        # Only show if there are formed patterns in the dataset
        if self.has_formed_patterns:
            dpoint_group = QGroupBox("D-Point Validation (Formed Only)")
            dpoint_group.setStyleSheet("QGroupBox { font-weight: bold; margin-top: 10px; color: #000; background-color: #e6f2ff; border: 2px solid #0066cc; border-radius: 5px; } QGroupBox::title { subcontrol-origin: margin; padding: 5px; color: #0066cc; font-weight: bold; }")
            dpoint_layout = QVBoxLayout(dpoint_group)

            # D-Point validation checkbox
            dpoint_widget = QWidget()
            dpoint_row = QHBoxLayout(dpoint_widget)
            dpoint_row.setSpacing(5)
            dpoint_row.setContentsMargins(8, 4, 8, 4)
            dpoint_widget.setStyleSheet("QWidget { background-color: #ffffff; border-radius: 3px; }")

            self.validate_d_crossing_checkbox = QCheckBox("Strict Validation")
            # Use parent's setting instead of always defaulting to True
            self.validate_d_crossing_checkbox.setChecked(self.parent_validate_d_crossing)
            self.validate_d_crossing_checkbox.setStyleSheet("QCheckBox { color: #000; font-weight: bold; font-size: 13px; background-color: transparent; }")
            self.validate_d_crossing_checkbox.setToolTip(
                "Applies to FORMED patterns only:\n\n"
                "✓ ENABLED (checked):\n"
                "  • Shows only patterns where D held\n"
                "  • Best for backtesting\n\n"
                "✗ DISABLED (unchecked):\n"
                "  • Shows all patterns, even if D crossed\n"
                "  • Best for live trading\n\n"
                "Click to toggle and re-detect"
            )
            self.validate_d_crossing_checkbox.stateChanged.connect(self.onDPointValidationChanged)

            status_label = QLabel("(Active)")
            status_label.setObjectName("dpoint_status")
            status_label.setStyleSheet("color: #0066cc; font-weight: bold; font-size: 11px; margin-left: 5px; background-color: transparent;")

            dpoint_row.addWidget(self.validate_d_crossing_checkbox)
            dpoint_row.addWidget(status_label)
            dpoint_row.addStretch()
            dpoint_layout.addWidget(dpoint_widget)

            filter_layout.addWidget(dpoint_group)

        # Pattern type filter section
        self.pattern_group = QGroupBox("Pattern Variations")
        self.pattern_group.setStyleSheet("QGroupBox { font-weight: bold; margin-top: 10px; color: #000; background-color: #fff; } QGroupBox::title { subcontrol-origin: margin; padding: 5px; color: #000; font-weight: bold; }")
        ...
        # Pattern types will be populated dynamically after analyzing patterns
        # This is just a placeholder - actual types are created in createPatternTypeCheckboxes()
        pattern_types = []
        ...
        for i, ptype in enumerate(pattern_types):
            ...
            self.pattern_layout.addWidget(row_widget)
    ...
    def updatePatternAxisLabels(self, x, y):
        ...
        try:
            # Use display_data if available (what's actually shown on chart)
    ...
    def refreshCurrentPattern(self):
        """Refresh the current pattern display (used when toggling Fibonacci levels)"""
        if hasattr(self, 'current_index') and self.filtered_patterns:
            self.showPattern(self.current_index)

    def applyFibonacciLevels(self):
        """Apply custom Fibonacci levels from input"""
        try:
            # Parse the input
            input_text = self.fib_levels_input.text().strip()
            if not input_text:
                QMessageBox.warning(self, "Invalid Input", "Please enter at least one Fibonacci level.")
                return

            # Split by comma and parse each level
            level_strings = [s.strip() for s in input_text.split(',')]
            levels = []
                ...
                try:
                    ...
                    if level < 0 or level > 200:
                        QMessageBox.warning(
                            self,
                            "Invalid Level",
                            f"Level {level}% is out of range. Please use values between 0 and 200."
                        )
                        return
                    levels.append(level)
                except ValueError:
                    QMessageBox.warning(
                        self,
                        "Invalid Format",
                        f"'{level_str}' is not a valid number. Please use format: 0, 23.6, 38.2, 50, 61.8, 100"
                    )
                    return
            ...
            # Store the custom levels (will be used by drawFibonacciLevels)
            self.custom_fib_levels = levels

            # Refresh the pattern
            self.refreshCurrentPattern()

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error parsing Fibonacci levels: {str(e)}")

    def loadFibPreset(self, preset_name):
        """Load a preset Fibonacci level configuration"""
        presets = {
            'standard': ['0', '23.6', '38.2', '50', '61.8', '78.6', '100'],
            'extended': ['0', '23.6', '38.2', '50', '61.8', '78.6', '100', '127.2', '161.8'],
            'minimal': ['0', '38.2', '50', '61.8', '100']
        }

        if preset_name in presets:
            self.fib_levels_input.setText(', '.join(presets[preset_name]))
            self.applyFibonacciLevels()
    ...
    def drawPattern(self, pattern):
        ...
        if self.extremum_points:
            for ep_time, ep_price, is_high, bar_idx in self.extremum_points:
        ...
        if len(x_coords) >= 2:
            ...
            for i, (x, y, label) in enumerate(zip(x_coords, y_coords, labels)):
                ...
                if point_name in points and 'time' in points[point_name]:
                    ...
                    # Fallback if time info not available
                    label_text = f"{label}\n${y:.2f}"
            ...
            # Draw Fibonacci Retracement Levels if enabled
            if hasattr(self, 'show_fibonacci_checkbox') and self.show_fibonacci_checkbox.isChecked():
        ...
        if 'D_projected' in points:
            ...
            if len(x_coords) > 0:
                ...
                if 'prz_zones' in d_projected:
                    ...
                    # Get C point time for context (since D is projected from C)
                    c_time_str = ""
                    if 'C' in points and 'time' in points['C']:
                        c_time = points['C']['time']
                        if hasattr(c_time, 'strftime'):
                            # Only show time if it's not all zeros
                            if c_time.hour == 0 and c_time.minute == 0 and c_time.second == 0:
                                c_time_str = c_time.strftime('%d %b')
                            else:
                                c_time_str = c_time.strftime('%d %b %H:%M')
                        else:
                            c_time = pd.to_datetime(c_time)
                            if c_time.hour == 0 and c_time.minute == 0 and c_time.second == 0:
                                c_time_str = c_time.strftime('%d %b')
                            else:
                                c_time_str = c_time.strftime('%d %b %H:%M')
                    ...
                    for i, d_price in enumerate(d_lines, 1):
                        ...
                        # Add label for each line with C point time
                        if c_time_str:
                            label_text = f"D{i}: {d_price:.2f}\n(from C: {c_time_str})"
                        else:
                            label_text = f"D{i}: {d_price:.2f}"
        ...
        # Handle PRZ zones for formed patterns (stored at top level)
        elif 'D' in points and 'prz_zones' in pattern:
            prz_zones = pattern['prz_zones']
            if len(x_coords) > 0:
                # Use the D point position for PRZ display
                d_x = x_coords[-1]  # D is the last point
                color = '#00BFFF' if 'bull' in pattern['name'].lower() else '#FF8C00'

                # Draw PRZ zones for formed patterns
                for i, zone in enumerate(prz_zones, 1):
                    # Draw zone as a shaded rectangle at D's position
                    zone_color = QColor(color)
                    zone_color.setAlpha(80)  # Semi-transparent
                    zone_brush = pg.mkBrush(zone_color)

                    # Create a rectangular zone around D point
                    rect = pg.QtWidgets.QGraphicsRectItem(d_x - 10, zone['min'],
                                                         20, zone['max'] - zone['min'])
                    rect.setBrush(zone_brush)
                    rect.setPen(pg.mkPen(color, width=2))
                    self.chart.addItem(rect)

                    # Add label with pattern source
                    pattern_source = zone.get('pattern_source', 'Unknown')
                    if 'AB=CD_bull_' in pattern_source:
                        simplified_name = 'ABCD Bull ' + pattern_source.replace('AB=CD_bull_', '')
                    elif 'AB=CD_bear_' in pattern_source:
                    ...
                    # Get D point time (for formed patterns, D is actual completion time)
                    d_time_str = ""
                    if 'D' in points and 'time' in points['D']:
                        d_time = points['D']['time']
                        if hasattr(d_time, 'strftime'):
                            # Only show time if it's not all zeros
                            if d_time.hour == 0 and d_time.minute == 0 and d_time.second == 0:
                                d_time_str = f"\nAt D: {d_time.strftime('%d %b %Y')}"
                            else:
                                d_time_str = f"\nAt D: {d_time.strftime('%d %b %H:%M')}"
                        else:
                            d_time = pd.to_datetime(d_time)
                            if d_time.hour == 0 and d_time.minute == 0 and d_time.second == 0:
                                d_time_str = f"\nAt D: {d_time.strftime('%d %b %Y')}"
                            else:
                                d_time_str = f"\nAt D: {d_time.strftime('%d %b %H:%M')}"

                    label_text = f"PRZ ({simplified_name})\n{zone['min']:.2f}-{zone['max']:.2f}{d_time_str}"
                    text = pg.TextItem(label_text, color=color, anchor=(0, 0.5))
                    text.setPos(d_x + 12, (zone['min'] + zone['max']) / 2)
                    text.setFont(QFont('Arial', 7, QFont.Weight.Bold))
                    self.chart.addItem(text)

        # Handle d_lines for formed XABCD patterns (stored at top level)
        elif 'D' in points and 'd_lines' in pattern and 'X' in points:
            d_lines = pattern['d_lines']
            if len(x_coords) > 0:
                # Use the D point position for d_lines display
                d_x = x_coords[-1]  # D is the last point
                color = '#00BFFF' if 'bull' in pattern['name'].lower() else '#FF8C00'
                ...
                if 'D' in points and 'time' in points['D']:
                    ...
                    if hasattr(d_time, 'strftime'):
                        # Only show time if it's not all zeros
                        if d_time.hour == 0 and d_time.minute == 0 and d_time.second == 0:
                            d_time_str = d_time.strftime('%d %b')
                        else:
                            d_time_str = d_time.strftime('%d %b %H:%M')
                    else:
                        d_time = pd.to_datetime(d_time)
                        if d_time.hour == 0 and d_time.minute == 0 and d_time.second == 0:
                            d_time_str = d_time.strftime('%d %b')
                ...
                # Draw d_lines for formed XABCD patterns
                for i, d_price in enumerate(d_lines, 1):
                    # Draw horizontal line for each D projection
                    line_pen = pg.mkPen(color=color, width=1.5, style=Qt.PenStyle.DashLine)
                    self.chart.plot([d_x - 8, d_x + 8], [d_price, d_price], pen=line_pen)

                    # Add label for each line with D point time
                    if d_time_str:
                        label_text = f"D{i}: {d_price:.2f}\n(at D: {d_time_str})"
                    else:
                        label_text = f"D{i}: {d_price:.2f}"
    ...
    def drawFibonacciLevels(self, points, x_coords, y_coords, labels, is_bullish):
        """
        Draw Fibonacci retracement levels on FORMED patterns only

        For BEARISH: From highest formation point (A or C) DOWN to lowest swing in PRZ (D point)
        For BULLISH: From lowest formation point (A or C) UP to highest swing in PRZ (D point)

        Golden Ratio zone: 0.5 - 0.618
        """
        # Only draw Fibonacci for FORMED patterns (must have D point, not D_projected)
        if 'D' not in points or 'D_projected' in points:
            return

        # Get custom levels if they exist, otherwise use defaults
        if hasattr(self, 'custom_fib_levels') and self.custom_fib_levels:
            # Use custom levels (already as percentages)
            fib_levels = [level / 100.0 for level in self.custom_fib_levels]
            fib_labels = [f'{level}%' for level in self.custom_fib_levels]
        else:
            # Use default levels
        ...
        # Determine the highest/lowest formation point (A or C)
    ...
    def _drawFibLevels(self, start_x, end_x, start_price, end_price, fib_levels, fib_labels, is_bullish, move_label):
        """Helper method to draw Fibonacci levels with Golden Ratio zone highlight"""
        price_range = end_price - start_price
        x_range = end_x - start_x

        # Color scheme - using purple/magenta tones to distinguish from PRZ (blue) and d_lines (cyan/green)
        # Line colors
        fib_line_color = '#9370DB'  # Medium Purple for lines
        golden_line_color = '#FFD700'  # Gold for golden ratio lines

        # Label colors - using bright, contrasting colors with black background for readability
        normal_label_color = '#FFFFFF'  # White text for normal levels
        golden_label_color = '#FFD700'  # Gold text for golden ratio

        # Golden Ratio zone color (0.5 - 0.618)
        golden_zone_color = QColor('#FFD700')  # Gold color
        golden_zone_color.setAlpha(50)

        # Check if golden ratio levels are enabled
        has_50 = 0.5 in fib_levels
        has_618 = 0.618 in fib_levels

        # Draw golden zone rectangle only if both 50% and 61.8% are enabled
        if has_50 and has_618:
            fib_50_price = start_price + (price_range * 0.5)
            fib_618_price = start_price + (price_range * 0.618)

            line_start_x = start_x
            line_end_x = end_x + (x_range * 0.3)
            zone_height = fib_618_price - fib_50_price
            golden_rect = pg.QtWidgets.QGraphicsRectItem(
                line_start_x, fib_50_price,
                line_end_x - line_start_x, zone_height
            )
            golden_rect.setBrush(pg.mkBrush(golden_zone_color))
            golden_rect.setPen(pg.mkPen(None))  # No border
            self.chart.addItem(golden_rect)

        # Line extension
        line_start_x = start_x
        line_end_x = end_x + (x_range * 0.3)

        # Now draw Fibonacci levels
        for level, label in zip(fib_levels, fib_labels):
            ...
            # Highlight Golden Ratio levels (50% and 61.8%)
            is_golden = level in [0.5, 0.618]

            if is_golden:
                # Thicker, solid lines for golden ratio
                pen = pg.mkPen(color=golden_line_color, width=2, style=Qt.PenStyle.SolidLine)
                text_color = golden_label_color
                font_weight = QFont.Weight.Bold
                label_suffix = " (Golden)"
            else:
                # Dashed lines for normal Fibonacci levels
                pen = pg.mkPen(color=fib_line_color, width=1.5, style=Qt.PenStyle.DashLine)
                text_color = normal_label_color
                font_weight = QFont.Weight.Bold
                label_suffix = ""

            # Draw horizontal line
            self.chart.plot([line_start_x, line_end_x], [fib_price, fib_price], pen=pen)

            # Add label with background for better visibility
            label_text = f"{move_label} {label}{label_suffix}"

            # Create text item with semi-transparent black background
            text = pg.TextItem(
                label_text,
                color=text_color,
                anchor=(0, 0.5),
                fill=pg.mkBrush(0, 0, 0, 180),  # Black background with transparency
                border=pg.mkPen(text_color if is_golden else fib_line_color, width=1)
            )
            text.setPos(line_end_x + 2, fib_price)
            text.setFont(QFont('Arial', 9, font_weight))
            self.chart.addItem(text)
    ...
    def updateDetails(self, pattern):
        ...
        if 'ratios' in pattern and 'matching_patterns' in pattern['ratios']:
            ...
            details = f"📊 {total_patterns} Patterns (ABCD {pattern_names_str})\n"
        else:
        ...
        # Remove Type line since bull/bear is already in pattern name

        # Show point values with dates/times
        if 'points' in pattern:
            points = pattern['points']
            # Determine which points to show based on pattern type
            if 'xabcd' in pattern.get('type', '').lower() or 'formed_xabcd' in pattern.get('pattern_type', ''):
            ...
            for point_name in point_names:
                if point_name in points and 'price' in points[point_name]:
                    ...
                    if 'time' in points[point_name]:
                        point_time = points[point_name]['time']
                        if hasattr(point_time, 'strftime'):
                            # Only show time if it's not all zeros
                            if point_time.hour == 0 and point_time.minute == 0 and point_time.second == 0:
                                time_str = point_time.strftime('%d %b')
                            else:
                                time_str = point_time.strftime('%d %b %H:%M')
                        else:
                            point_time = pd.to_datetime(point_time)
                            if point_time.hour == 0 and point_time.minute == 0 and point_time.second == 0:
                                time_str = point_time.strftime('%d %b')
                        ...
                        point_values.append(f"{point_name}=${price:.2f}")

            # Collect ratios to append to the same line
            ratios_str = ""
            if 'ratios' in pattern:
                ...
                if 'ad_xa_range' in ratios and isinstance(ratios['ad_xa_range'], (list, tuple)):
                    ...
                    ratio_values.append(f"AD/XA: {ad_range[0]:.1f}%-{ad_range[1]:.1f}%")
                if 'cd_bc_range' in ratios and isinstance(ratios['cd_bc_range'], (list, tuple)):
                    cd_range = ratios['cd_bc_range']
                    ratio_values.append(f"CD/BC: {cd_range[0]:.1f}%-{cd_range[1]:.1f}%")
                ...
                if 'bc_retracement' in ratios and isinstance(ratios['bc_retracement'], (int, float)):
                    ratio_values.append(f"BC: {ratios['bc_retracement']:.1f}%")

                if ratio_values:
                    ratios_str = " | Ratios: " + "  •  ".join(ratio_values)

            if point_values:
                details += "Points: " + " • ".join(point_values) + ratios_str + "\n"
            ...
            if 'D_projected' in points:
                ...
                if 'prz_zones' in d_proj:
                    ...
                    for i, zone in enumerate(d_proj['prz_zones'], 1):
                        ...
                        # Check if proj_min and proj_max exist in zone
                    ...
                    details += f"\nD Projections ({len(d_lines)} lines, from C @ {c_time_str}):\n" if c_time_str else f"\nD Projections ({len(d_lines)} lines):\n"
                    for i, d_price in enumerate(d_lines, 1):
                        details += f"  D{i}: ${d_price:.2f}\n"
                elif 'price' in d_proj:
                    details += f"  D_projected: ${d_proj['price']:.2f}\n"

        # Show PRZ zones for formed ABCD patterns (stored at top level)
        if 'prz_zones' in pattern and 'D' in pattern.get('points', {}) and 'X' not in pattern.get('points', {}):
            prz_list = []
            proj_list = []
            for i, zone in enumerate(pattern['prz_zones'], 1):
                prz_list.append(f"{i}:${zone['min']:.2f}-${zone['max']:.2f}")
                # Check if proj_min and proj_max exist in zone
                if 'proj_min' in zone and 'proj_max' in zone:
                    proj_list.append(f"{zone['proj_min']:.1f}%-{zone['proj_max']:.1f}%")
        ...
        # Show d_lines for formed XABCD patterns (stored at top level)
        if 'd_lines' in pattern and 'D' in pattern.get('points', {}) and 'X' in pattern.get('points', {}):
            d_lines = pattern['d_lines']
            # Get D point time for formed patterns
            ...
            details += f"D Lines ({len(d_lines)} lines, at D @ {d_time_str}):\n" if d_time_str else f"D Lines (Formed XABCD Pattern - {len(d_lines)} lines):\n"
            for i, d_price in enumerate(d_lines, 1):
                details += f"  D{i}: ${d_price:.2f}\n"
    ...
    def analyzePatterns(self):
        ...
        for pattern in self.all_patterns:
            ...
            # Extract base pattern type (remove _bull/_bear suffix)
            pattern_name = pattern['name']
            base_name = pattern_name.replace('_bull', '').replace('_bear', '')

            # Count this pattern type
            if base_name not in self.pattern_type_counts:
                self.pattern_type_counts[base_name] = 0
            self.pattern_type_counts[base_name] += 1

        # Now create checkboxes for all detected pattern types
        self.createPatternTypeCheckboxes()

    def createPatternTypeCheckboxes(self):
        """Dynamically create checkboxes for all detected pattern types"""
        # Clear existing checkboxes
        while self.pattern_layout.count():
            child = self.pattern_layout.takeAt(0)
            if child.widget():
        ...
        # Sort pattern types for consistent display
        sorted_ptypes = sorted(self.pattern_type_counts.keys())

        for i, ptype in enumerate(sorted_ptypes):
            # Create a container widget for the row
            row_widget = QWidget()
            pattern_row = QHBoxLayout(row_widget)
            pattern_row.setSpacing(5)
            pattern_row.setContentsMargins(8, 4, 8, 4)

            # Alternate row colors
            if i % 2 == 0:
                row_widget.setStyleSheet("QWidget { background-color: #f0f8ff; border-radius: 3px; }")
            else:
                row_widget.setStyleSheet("QWidget { background-color: #f5f5f5; border-radius: 3px; }")

            # Create checkbox
            checkbox = QCheckBox(f"{ptype}")
            checkbox.setChecked(True)
            checkbox.setStyleSheet("QCheckBox { color: #000; font-weight: bold; font-size: 12px; background-color: transparent; }")
            checkbox.stateChanged.connect(self.applyFilters)

            # Create count label
            count = self.pattern_type_counts[ptype]
            label = QLabel(f"({count}/{count})")
            if i % 2 == 0:
                label.setStyleSheet("color: #0066cc; font-weight: bold; font-size: 12px; margin-left: 5px; background-color: transparent;")
            else:
                label.setStyleSheet("color: #666666; font-weight: bold; font-size: 12px; margin-left: 5px; background-color: transparent;")
    ...
    def updateFilterCounts(self):
        """Update the filter labels with current counts - initial load, all patterns visible"""
        # Show total/total since all patterns are visible initially
        self.bull_label.setText(f"({self.bull_count}/{self.bull_count})")
        self.bear_label.setText(f"({self.bear_count}/{self.bear_count})")

        for ptype, count in self.pattern_type_counts.items():
            if ptype in self.pattern_labels:
                # Show total/total since all patterns are visible initially
                self.pattern_labels[ptype].setText(f"({count}/{count})")
    ...
    def applyFilters(self):
        ...
        for pattern in self.all_patterns:
            ...
            if not selected_pattern_types:  # If no types selected, show none
                ...
                # Extract base name from this pattern
                base_name = pattern['name'].replace('_bull', '').replace('_bear', '')
    ...
    def updateFilteredCounts(self):
        ...
        # Count bulls/bears that would be visible with current pattern type filters
        # This shows bidirectional filtering: both filters affect both counts
        show_bulls = self.bull_checkbox.isChecked()
        show_bears = self.bear_checkbox.isChecked()
        ...
        for pattern in self.all_patterns:
            ...
            is_bear = 'bear' in pattern_name.lower()

            # Check if pattern type matches selected types
            base_name = pattern_name.replace('_bull', '').replace('_bear', '')
            pattern_type_matches = (base_name in selected_pattern_types) if selected_pattern_types else False

            # Only count if pattern type filter passes
        ...
        # Update bull/bear labels with available count
        self.bull_label.setText(f"({bull_available}/{self.bull_count})")
        self.bear_label.setText(f"({bear_available}/{self.bear_count})")

        # Update pattern type counts - count patterns that pass bull/bear filter
        ...
        for pattern in self.all_patterns:
            ...
            # Check if this pattern passes bull/bear filter
            bull_bear_pass = False
            if show_bulls and is_bull:
                bull_bear_pass = True
            elif show_bears and is_bear:
            ...
            # Only count if bull/bear filter passes
            if bull_bear_pass:
                base_name = pattern_name.replace('_bull', '').replace('_bear', '')
                if base_name in pattern_filtered_counts:
                    pattern_filtered_counts[base_name] += 1

        for ptype, total_count in self.pattern_type_counts.items():
            filtered_count = pattern_filtered_counts[ptype]
            if ptype in self.pattern_labels:
                # Keep consistent format: (filtered/total)
                self.pattern_labels[ptype].setText(f"({filtered_count}/{total_count})")

    def onDPointValidationChanged(self):
        """Handle D-Point validation checkbox change"""
        from PyQt6.QtWidgets import QMessageBox
        ...
        if is_strict:
            msg.setText(f"Mode: {mode}\n\n"
                       "Will show only formed patterns where D held.\n\n"
                       "Re-run pattern detection now?")
        else:
            msg.setText(f"Mode: {mode}\n\n"
                       "Will show ALL formed patterns (even if D crossed).\n\n"
                       "Re-run pattern detection now?")
        ...
        if result == QMessageBox.StandardButton.Yes:
            ...
            if parent:
                # Store the setting
                parent.validate_d_crossing_setting = is_strict
                # Close this window
                self.close()
                # Trigger re-detection
                parent.detectPatterns()
...
class HarmonicPatternDetector(QMainWindow):
    ...
    def __init__(self):
        ...
        # Initialize watchlist manager and auto-update scheduler
        self.watchlist_manager = WatchlistManager()
        self.auto_updater = None  # Will be initialized after UI is ready
        ...
        # Start auto-update scheduler after UI is initialized
    ...
    def initUI(self):
        ...
        """)
        watchlist_layout.addWidget(self.open_watchlist_btn)
        ...
        self.timeframe_combo.setEditable(True)  # Allow custom input
        ...
        self.timeframe_combo.setToolTip(
            "Enter ANY timeframe in format: {number}{unit}\n\n"
            "Examples:\n"
            "  Minutes: 1m, 2m, 10m, 45m, 90m\n"
            "  Hours: 1h, 3h, 5h, 24h\n"
            "  Days: 1d, 2d, 5d, 7d\n"
            "  Weeks: 1w, 2w\n"
            "  Months: 1M, 3M\n\n"
            "System will automatically:\n"
            "  1. Download base data from Binance\n"
            "  2. Resample to your custom timeframe\n"
            "Works exactly like TradingView!"
        )
        ...
        self.formed_abcd_checkbox = QCheckBox("Formed ABCD")
        self.formed_abcd_checkbox.setToolTip("Detect complete ABCD patterns with all 4 points")
        pattern_layout.addWidget(self.formed_abcd_checkbox)

        self.formed_xabcd_checkbox = QCheckBox("Formed XABCD")
        self.formed_xabcd_checkbox.setToolTip("Detect complete XABCD patterns with all 5 points")
        pattern_layout.addWidget(self.formed_xabcd_checkbox)
        ...
        search_window_group = QGroupBox("Search Window")
        search_window_group.setToolTip(
            "Search Window controls the maximum distance (in bars) between pattern points.\n\n"
            "What it does:\n"
            "  • Limits how far apart consecutive points (A→B, B→C, C→D) can be\n"
            "  • Lower values = tight, compact patterns (more precise)\n"
            "  • Higher values = wider, looser patterns (more patterns found)\n\n"
            "Example: If set to 5, point B must be within 5 bars of point A"
        )
        search_window_vlayout = QVBoxLayout()

        # Radio buttons for preset options
        self.search_window_optimum_radio = QRadioButton("Optimum (Auto - Recommended)")
        self.search_window_optimum_radio.setToolTip("System automatically determines optimal distance based on dataset size")
        # Don't set as checked - force user to select

        self.search_window_minimum_radio = QRadioButton("Minimum (1 bar)")
        self.search_window_minimum_radio.setToolTip("Tightest patterns - consecutive points must be adjacent (1 bar apart)")

        self.search_window_maximum_radio = QRadioButton("Maximum (9 bars)")
        self.search_window_maximum_radio.setToolTip("Widest patterns - allows up to 9 bars between consecutive points")

        # Custom value option with input field
        custom_layout = QHBoxLayout()
        self.search_window_custom_radio = QRadioButton("Custom:")
        self.search_window_custom_radio.setToolTip("Enter your own value between 1-9")
        custom_layout.addWidget(self.search_window_custom_radio)

        self.search_window_custom_spinbox = QSpinBox()
        self.search_window_custom_spinbox.setMinimum(1)
        self.search_window_custom_spinbox.setMaximum(9)
        self.search_window_custom_spinbox.setValue(5)
        self.search_window_custom_spinbox.setEnabled(False)  # Disabled until custom radio is selected
        self.search_window_custom_spinbox.setToolTip("Enter custom value (1-9 bars)")
        self.search_window_custom_spinbox.setMaximumWidth(60)
        custom_layout.addWidget(self.search_window_custom_spinbox)
        custom_layout.addWidget(QLabel("bars"))
        custom_layout.addStretch()

        # Enable/disable spinbox based on radio selection
        def onCustomRadioToggled(checked):
            self.search_window_custom_spinbox.setEnabled(checked)
        ...
        search_window_vlayout.addWidget(self.search_window_maximum_radio)
        search_window_vlayout.addLayout(custom_layout)

        search_window_group.setLayout(search_window_vlayout)
        pattern_layout.addWidget(search_window_group)
        ...
        # Detect Patterns button with built-in progress indicator
        ...
        # No custom styling - will match Run Backtesting button
        pattern_layout.addWidget(self.detect_patterns_btn)

        # Add Backtesting button (placed after progress bar)
        ...
        # Initialize toast notification manager (popups only, no dock)
        self.toast_manager = ToastManager(self, max_toasts=5)

        # Watchlist window reference (created on demand)
    ...
    def initializeAutoUpdater(self):
        ...
        try:
            ...
            def progress_callback(chart, percent, message):
                # You can update a progress bar here if needed
                pass

            # Create notification callback for failed updates
            def notification_callback(title, message, notification_type):
                try:
                    if hasattr(self, 'toast_manager'):
                        if notification_type == 'error':
                            self.toast_manager.notify_error(title, message)
                        elif notification_type == 'warning':
                            self.toast_manager.notify_warning(title, message)
                        elif notification_type == 'success':
                            self.toast_manager.notify_success(title, message)
                        else:
                            self.toast_manager.notify_info(title, message)
                except:
                    pass  # Ignore errors in callback

            # Initialize auto-updater with longer check interval to avoid interference
            self.auto_updater = AutoUpdateScheduler(
                self.watchlist_manager,
                check_interval=600,  # Check every 10 minutes (increased from 5)
            ...
            if hasattr(self, 'watchlist_panel'):
                self.watchlist_panel.auto_updater = self.auto_updater
            ...
            # Show initial status
            summary = self.watchlist_manager.get_status_summary()
            print(f"Watchlist summary: {summary}")
            self.status_bar.showMessage(
                f"Auto-update enabled: {summary['enabled_charts']} charts in watchlist",
                3000
            )

            # Show welcome notification
            if hasattr(self, 'toast_manager'):
                self.toast_manager.notify_success(
                    "Auto-Update Enabled",
            ...
            print(traceback.format_exc())
            self.status_bar.showMessage(f"Auto-update initialization failed: {e}", 5000)
    ...
    def downloadBinanceData(self):
        """Download data from Binance API with support for custom timeframes (non-blocking)"""
        try:
            # Get parameters
            symbol = self.symbol_combo.currentText().strip().upper()
            target_timeframe = self.timeframe_combo.currentText().strip()

            # Validate timeframe format
            import re
            if not re.match(r'^(\d+)([mhdwM])$', target_timeframe):
                QMessageBox.warning(
                    self,
                    "Invalid Timeframe Format",
                    f"Invalid timeframe format: '{target_timeframe}'\n\n"
                    f"Valid format: {{number}}{{unit}}\n"
                    f"Examples: 1m, 5m, 1h, 2d, 1w\n\n"
                    f"Units:\n"
                    f"  m = minutes\n"
                    f"  h = hours\n"
                    f"  d = days\n"
            ...
            self.download_worker = DataDownloadWorker(symbol, target_timeframe, start_date, end_date)

            # Connect signals
            self.download_worker.progress.connect(self.onDownloadProgress)
            self.download_worker.finished.connect(self.onDownloadFinished)
            self.download_worker.error.connect(self.onDownloadError)
    ...
    def onDownloadProgress(self, percent, message):
    ...
    def onDownloadFinished(self, df, symbol, timeframe):
        ...
        try:
            print(f"Download finished, processing {len(df)} candles...")
            ...
            downloader = BinanceDataDownloader()
            downloader.save_to_csv(df, file_path)
            print("CSV saved successfully")

            # Add to watchlist for auto-updates
            ...
            # Process data asynchronously using QTimer to avoid blocking
            def processData():
                try:
                    print(">>> Step 1: Starting standardizeDataFormat...")
                    self.download_status.setText("Processing data...")
                    QApplication.processEvents()

                    self.data = self.standardizeDataFormat(df.copy())
                    print(">>> Step 2: Data standardized OK")
                    QApplication.processEvents()

                    print(">>> Step 3: Updating file label...")
                    self.file_label.setText(f"Data: {symbol} {timeframe}")
                    print(">>> Step 4: Updating status bar...")
                    self.status_bar.showMessage(f"Downloaded {len(self.data)} candles for {symbol}")
                    QApplication.processEvents()

                    print(">>> Step 5: Setting chart title...")
                    self.main_chart.setTitle(f"{symbol} - {timeframe.upper()} Candlestick Chart")
                    QApplication.processEvents()

                    print(">>> Step 6: Setting date range controls...")
                    ...
                    print(">>> Step 7: Date range set OK")
                    QApplication.processEvents()

                    print(">>> Step 8: Enabling controls...")
                    self.clip_btn.setEnabled(True)
                    self.detect_extremums_btn.setEnabled(True)
                    self.backtest_btn.setEnabled(True)

                    print(">>> Step 9: Clearing patterns...")
                    self.extremum_points = []
                    self.detected_patterns = {}
                    QApplication.processEvents()

                    print(">>> Step 10: Skipping plot...")
                    # DON'T plot immediately - too heavy, causes freeze

                    print(">>> Step 11: Updating statistics...")
                    self.updateStatistics()
                    print(">>> Step 12: Statistics updated OK")
                    QApplication.processEvents()

                    print(">>> Step 13: Setting success message...")
                    self.download_status.setText(f"✓ Downloaded {len(df)} candles. Click 'Clip Data' to display chart.")
                    self.status_bar.showMessage(f"Added {symbol} {timeframe} to watchlist for auto-updates", 3000)

                    # Show success notification
                    if hasattr(self, 'toast_manager'):
                        self.toast_manager.notify_success(
                            f"Downloaded: {symbol} {timeframe}",
                            f"Successfully downloaded {len(df)} candles and added to watchlist for auto-updates."
                        )

                    print(">>> Step 14: Re-enabling button...")
                    self.download_btn.setEnabled(True)
                    QTimer.singleShot(2000, lambda: self.download_progress.setVisible(False))
                    print(">>> DOWNLOAD COMPLETE! All steps finished successfully.")

                except Exception as e:
                    import traceback
                    error_details = f"Failed to process data: {str(e)}\n{traceback.format_exc()}"
                    print(error_details)
                    QMessageBox.critical(self, "Error", f"Failed to process downloaded data:\n\n{str(e)}")
                    self.download_btn.setEnabled(True)
                    self.download_progress.setVisible(False)

            # Schedule data processing on next event loop iteration
            print("Scheduling data processing in 100ms...")
            QTimer.singleShot(100, processData)

        except Exception as e:
            import traceback
            error_details = f"Failed to save downloaded data: {str(e)}\n\nDetails:\n{traceback.format_exc()}"
            print(error_details)
            QMessageBox.critical(self, "Download Error", f"Failed to save data:\n\n{str(e)}")
    ...
    def detectExtremums(self):
        ...
        print("\n" + "="*60)
        print("EXTREMUM DETECTION STARTED")
        print("="*60)
        ...
        for i in range(length, n - length):
            ...
            # Unlike the old logic which chose one, we keep both to allow pattern detection flexibility
            if is_high_pivot:
                self.extremum_points.append((timestamps[i], highs[i], True, i))

            # Add low pivot separately (not elif) to capture both if they exist on same candle
            if is_low_pivot:
                self.extremum_points.append((timestamps[i], lows[i], False, i))

        # Sort by date
        self.extremum_points.sort(key=lambda x: x[0])

        # Count highs and lows separately for detailed reporting
        high_count = sum(1 for _, _, is_high, _ in self.extremum_points if is_high)
        low_count = len(self.extremum_points) - high_count

        # Debug: Check if specific bars are detected
        bar_indices = [idx for _, _, _, idx in self.extremum_points]
        ...
        print("="*60)
        print("EXTREMUM DETECTION FINISHED")
        print("="*60 + "\n")
    ...
    def addManualExtremum(self, point):
        ...
        if high_dist < low_dist:
            ...
            is_high = True
            if date.hour == 0 and date.minute == 0 and date.second == 0:
                self.status_bar.showMessage(f"Added HIGH at {date.strftime('%d %b %Y')}: {price:.2f}")
            else:
                self.status_bar.showMessage(f"Added HIGH at {date.strftime('%d %b %H:%M')}: {price:.2f}")
        else:
            # Add as low
            price = candle['Low']
            is_high = False
            if date.hour == 0 and date.minute == 0 and date.second == 0:
                self.status_bar.showMessage(f"Added LOW at {date.strftime('%d %b %Y')}: {price:.2f}")
            else:
                self.status_bar.showMessage(f"Added LOW at {date.strftime('%d %b %H:%M')}: {price:.2f}")
        ...
        # Add new extremum with bar index
        self.extremum_points.append((date, price, is_high, x_index))
        self.extremum_points.sort(key=lambda x: x[0])
    ...
    def removeNearestExtremum(self, point):
        ...
        if nearest_ep and min_dist < threshold:
            ...
            self.plotExtremums()
            self.updateStatistics()
            # Format date in TradingView style
            ep_date = nearest_ep[0]
            if hasattr(ep_date, 'strftime'):
                if ep_date.hour == 0 and ep_date.minute == 0 and ep_date.second == 0:
                    date_str = ep_date.strftime('%d %b %Y')
                else:
                    date_str = ep_date.strftime('%d %b %H:%M')
            else:
                date_str = str(ep_date)[:10]
    ...
    def detectPatterns(self):
        ...
        # VALIDATION 1: Check if search window is selected
        search_window_value = None
        if hasattr(self, 'search_window_optimum_radio'):
            if self.search_window_optimum_radio.isChecked():
                search_window_value = 0  # Auto
            elif self.search_window_minimum_radio.isChecked():
                search_window_value = 1  # Minimum
            elif self.search_window_maximum_radio.isChecked():
                search_window_value = 9  # Maximum
            elif self.search_window_custom_radio.isChecked():
                search_window_value = self.search_window_custom_spinbox.value()  # Custom value

            # If no radio button is selected, show error
            if search_window_value is None:
                QMessageBox.critical(
                    self,
                    "Search Window Not Selected",
                    "⚠️ Please select a Search Window option before detecting patterns!\n\n"
                    "Available Options:\n"
                    "  • Optimum (Auto) - Recommended for most cases\n"
                    "  • Minimum (1 bar) - Tightest patterns\n"
                    "  • Maximum (9 bars) - Widest patterns\n"
                    "  • Custom (1-9) - Your own value\n\n"
                    "The Search Window control is in the Pattern Detection section."
                )
                return
        ...
        print(f"UI updated, starting detection process...")
        ...
        if hasattr(self, 'formed_abcd_checkbox') and self.formed_abcd_checkbox.isChecked():
            pattern_types.append('formed_abcd')
        if hasattr(self, 'formed_xabcd_checkbox') and self.formed_xabcd_checkbox.isChecked():
            pattern_types.append('formed_xabcd')
        ...
        if not pattern_types:
            pattern_types = ['formed_abcd', 'formed_xabcd', 'comprehensive_abcd', 'comprehensive_xabcd']
            print("No patterns selected - detecting all pattern types")
        ...
        data_to_use = self.filtered_data if self.filtered_data is not None else self.data
        # Get validate_d_crossing setting (can be changed in popup window, defaults to True)
        validate_d_crossing = getattr(self, 'validate_d_crossing_setting', True)
        self.pattern_worker = PatternDetectionWorker(self.extremum_points, pattern_types, data_to_use, self, validate_d_crossing)
        self.pattern_worker.progress.connect(self.updateProgressBar)
        self.pattern_worker.status.connect(self.updateProgressStatus)
        self.pattern_worker.finished.connect(self.onPatternsDetected)

        # Disable button during detection and change text
        self.detect_patterns_btn.setEnabled(False)
        self.detect_patterns_btn.setText("Starting detection... 0%")
        if DEBUG_MODE: print("Starting pattern worker thread...")
        self.pattern_worker.start()

    def updateProgressBar(self, value):
        """Update button progress indicator and text"""
        # Update button gradient to show progress (matching default Qt button style)
        progress = value / 100.0
        self.detect_patterns_btn.setStyleSheet(f"""
            QPushButton {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #5CB85C, stop:{progress} #5CB85C,
                    stop:{progress} #E0E0E0, stop:1 #E0E0E0);
                color: black;
            }}
        ...
        if value == 0:
            ...
            self.detect_patterns_btn.setText(f"Detecting patterns... {value}%")
        else:
            self.detect_patterns_btn.setText("Detection complete! 100%")

    def updateProgressStatus(self, status):
        """Update status bar and button text with current status"""
        self.status_bar.showMessage(status)
        # Extract a short version of the status for button
        short_status = status.split(':')[0] if ':' in status else status
        if len(short_status) > 30:
            short_status = short_status[:27] + "..."
        # Update button text with status if we have a current progress value
        current_text = self.detect_patterns_btn.text()
        if "%" in current_text:
            percentage = current_text.split("%")[0].split()[-1]
            self.detect_patterns_btn.setText(f"{short_status}... {percentage}%")
    ...
    def onPatternsDetected(self, patterns):
        ...
        # Reset button style to default (remove custom progress styling)
        self.detect_patterns_btn.setStyleSheet("")
        ...
        # Show message box with results - always show all pattern types with proper naming
        pattern_type_names = {
            'formed_abcd': 'Formed ABCD',
            'formed_xabcd': 'Formed XABCD',
            'comprehensive_abcd': 'Unformed ABCD',
            'comprehensive_xabcd': 'Unformed XABCD',
            'abcd': 'ABCD',
            'xabcd': 'XABCD'
        }

        pattern_counts = []
        for ptype in ['formed_abcd', 'formed_xabcd', 'comprehensive_abcd', 'comprehensive_xabcd']:
            plist = patterns.get(ptype, [])
            display_name = pattern_type_names.get(ptype, ptype.upper())
            count = len(plist) if plist else 0
            pattern_counts.append(f"{display_name}: {count} patterns")

        if total_patterns > 0:
            # Count unique pattern types (names)
            unique_patterns = set()
            for plist in patterns.values():
                for pattern in plist:
                    if isinstance(pattern, dict) and 'name' in pattern:
                        unique_patterns.add(pattern['name'])

            # Always open pattern viewer window (removed show all patterns option)
            QMessageBox.information(
                self,
                "Pattern Detection Complete",
                f"Total Pattern Instances Detected: {total_patterns} from {len(unique_patterns)} unique pattern types.\n\n" +
                "\n".join(pattern_counts) +
    ...
    def saveExtremums(self):
        ...
        if file_path:
            extremums_data = [
                ...
                for i, ep in enumerate(self.extremum_points)
            ]
            ...
            try:
                ...
                self.extremum_points = [
                    (
                        ...
                        ep['price'],
                        ep['is_high'],
                        ep.get('bar_index', i)  # Use stored index or fallback to position
                    )
                    for i, ep in enumerate(extremums_data)
                ]
    ...
    def openBacktestDialog(self):
        ...
        if response == QMessageBox.StandardButton.Yes:
            # Always create a fresh dialog to avoid cached results
            # Close existing dialog if it exists
            if hasattr(self, 'backtesting_dialog') and self.backtesting_dialog:
                self.backtesting_dialog.close()
                self.backtesting_dialog.deleteLater()
    ...
    def closeEvent(self, event):
        ...
        try:
            # Stop auto-updater
            if hasattr(self, 'auto_updater') and self.auto_updater is not None:
                print("Stopping auto-update scheduler...")
            ...
            # Close any open pattern windows
            ...
            # Force Qt application to quit completely
            QApplication.instance().quit()

        except Exception as e:
            print(f"Error during cleanup: {e}")
            event.accept()
            # Still try to quit even if there's an error
            QApplication.instance().quit()
