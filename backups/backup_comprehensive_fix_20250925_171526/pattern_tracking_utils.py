...
class PatternTracker:
    ...
    def track_unformed_pattern(self, pattern: Dict, current_bar: int, current_timestamp: datetime = None) -> str:
        ...
        tracked = TrackedPattern(
            ...
            subtype=pattern.get('subtype', pattern.get('name', '')),
    ...
    def update_unformed_to_formed(self, formed_pattern: Dict, current_bar: int) -> Optional[str]:
        ...
        for pattern_id, tracked in self.tracked_patterns.items():
            ...
            # Check if pattern names are compatible (unformed vs formed versions)
            unformed_base = tracked.subtype.replace('_unformed', '')
            formed_base = formed_name.replace('_formed', '')
            ...
            if (tracked.a_point[0] == a_idx and
                ...
                print(f"  Type: {pattern_type}, Name: {formed_name}")
                print(f"  D point: bar {d_idx}, price {d_price:.2f}")

                return pattern_id

        # If no exact match found and this is XABCD, try alternative matching
        if pattern_type == 'XABCD':
            return self._try_xabcd_fallback_matching(formed_pattern, current_bar)

        return None

    def _try_xabcd_fallback_matching(self, formed_pattern: Dict, current_bar: int) -> Optional[str]:
        """
        Fallback matching for XABCD patterns when normal matching fails.
        This handles cases where formed XABCD patterns have different data structures.
        """
        print(f"DEBUG: Trying XABCD fallback matching for formed pattern at bar {current_bar}")

        # Get any pending XABCD patterns
        pending_xabcd = []
        for pattern_id, tracked in self.tracked_patterns.items():
            if (tracked.pattern_type == 'XABCD' and
                tracked.status == 'pending' and
                not tracked.actual_d_price):
                pending_xabcd.append((pattern_id, tracked))

        if not pending_xabcd:
            print("DEBUG: No pending XABCD patterns to match")
            return None

        print(f"DEBUG: Found {len(pending_xabcd)} pending XABCD patterns to try matching")

        # Get D point data from formed pattern
        points = formed_pattern.get('points', {})
        d_point = points.get('D')

        if not d_point:
            print("DEBUG: Formed XABCD pattern has no D point")
            return None

        # Try to extract D point info
        d_price = d_point.get('price', 0)
        d_idx = d_point.get('index', -1)

        # If we can't get valid D point data, skip
        if d_price <= 0:
            print(f"DEBUG: Invalid D point data: price={d_price}, index={d_idx}")
            return None

        # For fallback matching, if index is missing, use current bar
        if d_idx < 0:
            d_idx = current_bar
            print(f"DEBUG: D point index missing, using current bar: {d_idx}")

        # For fallback matching, use the most recent pending XABCD pattern
        # This is a reasonable assumption since patterns form sequentially
        pattern_id, tracked = pending_xabcd[-1]  # Use the most recently tracked

        print(f"DEBUG: Matching formed XABCD to most recent pending: {pattern_id[:20]}...")

        # Update the tracked pattern with D point
        tracked.d_point = (d_idx, d_price)
        tracked.actual_d_price = d_price
        tracked.actual_d_bar = d_idx
        tracked.formed_pattern_name = formed_pattern.get('name', 'Unknown')
        tracked.bars_from_c_to_zone = d_idx - tracked.c_point[0] if tracked.c_point else 0
        tracked.bars_from_a_to_d = d_idx - tracked.a_point[0] if tracked.a_point else 0

        print(f"Pattern {pattern_id[:8]}... transitioned from unformed to formed at bar {current_bar}")
        print(f"  Type: XABCD (fallback matching), Name: {formed_pattern.get('name')}")
        print(f"  D point: bar {d_idx}, price {d_price:.2f}")
