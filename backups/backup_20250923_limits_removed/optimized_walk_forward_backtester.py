...
class OptimizedWalkForwardBacktester:
    ...
    def detect_patterns_with_cache(self, current_idx: int) -> Tuple[List[Dict], List[Dict]]:
        ...
        # Detect formed patterns less frequently (every 20 bars)
        if current_idx % 20 == 0:
            try:
                # Formed patterns using GUI-compatible detection (ensures consistency)
                formed_abcd, formed_xabcd = detect_all_gui_patterns(extremum_points, data_slice, max_patterns=None)
    ...
    def run_backtest(self, progress_callback=None) -> BacktestStatistics:
        ...
        start_idx = 100  # Start with some historical data
        for idx in range(start_idx, len(self.data)):
            ...
            if unformed_patterns:
                ...
                for pattern in unformed_patterns:
                    pattern_id = f"{pattern.get('name', 'unknown')}_{pattern.get('C', 0)}"
                    unique_unformed_patterns_found.add(pattern_id)
                    # Track unformed pattern with PatternTracker
                    self.pattern_tracker.track_unformed_pattern(pattern, idx)
            if formed_patterns:
                total_formed_found += len(formed_patterns)
                # Update the tracker with formed patterns
                self.pattern_tracker.update_formed_patterns(formed_patterns, idx)
        ...
        print("PATTERN DETECTION SUMMARY")
        print("="*60)
        print(f"Total unformed patterns found: {total_unformed_found}")
        print(f"Unique unformed patterns identified: {len(unique_unformed_patterns_found)}")
        print(f"Total formed patterns found: {total_formed_found}")
        print(f"Unique patterns identified: {len(self.pattern_cache)}")
        ...
        # Store pattern counts for statistics
        self.total_unformed_found = total_unformed_found
        self.total_formed_found = total_formed_found
