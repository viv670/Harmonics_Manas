...
# Configuration constant for PRZ calculation
EPSILON = 1e-10


def calculate_prz_zones(b_price: float, c_price: float, pattern_name: str, is_bullish: bool) -> List[Dict]:
    """
    Calculate PRZ (Potential Reversal Zone) zones for ABCD patterns.
    This follows the same algorithm as comprehensive_abcd_patterns.py

    Args:
        b_price: Price at point B
        c_price: Price at point C
        pattern_name: The pattern name (e.g., 'Gartley_bull')
        is_bullish: Whether the pattern is bullish or bearish

    Returns:
        List of PRZ zone dictionaries with min/max prices and projection details
    """
    prz_zones = []

    # Get the pattern ratios
    if pattern_name not in ABCD_PATTERN_RATIOS:
        # Try without '_strict' suffix if present
        base_pattern_name = pattern_name.replace('_strict', '')
        if base_pattern_name not in ABCD_PATTERN_RATIOS:
            return []
        pattern_name = base_pattern_name

    ratios = ABCD_PATTERN_RATIOS[pattern_name]
    ...
    # Get projection range from pattern ratios
    proj_min = ratios['proj'][0]  # Minimum projection percentage
    proj_max = ratios['proj'][1]  # Maximum projection percentage
    ...
    prz_zones.append(prz_zone)

    # For multiple pattern matches, we could add more zones here
    # But for formed patterns, we typically have one specific pattern
            ...
            for b_idx, b_time, b_price in valid_b:
                ...
                for c_idx, c_time, c_price in valid_c:
                    ...
                    for d_idx, d_time, d_price in valid_d:
                        ...
                        # Calculate PRZ zones for this pattern
                        prz_zones = calculate_prz_zones(b_price, c_price, pattern_name, is_bullish)
                        ...
                        pattern = {
                            ...
                            'points': {
                                ...
                                'D_projected': {'prz_zones': prz_zones}  # Add PRZ zones like unformed patterns
                            },
                            'ratios': {
                                'bc_retracement': bc_retracement,
                                'cd_projection': cd_projection,
                                'prz_zones': prz_zones  # Also include in ratios for easy access
                            },
                            'indices': {
                                'A': a_idx,
                                'B': b_idx,
